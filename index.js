...
// console.log(tableContainer, buttons)

const tableElem = document.createElement('table');
function createTableSkeleton(newTableElem){//makes sures that thead is always ready with styling
    newTableElem.classList.add('marginTopAndBottom');
    const thead = newTableElem.createTHead();
    // console.log(thead)
    ...
    headings.forEach((heading) =>{
        const colHead = rowHead.insertCell();
        if(count == 0){
            colHead.classList.add('pad-1');
        }else if(count == 1){
            colHead.classList.add('pad-2');
        }else{
            colHead.classList.add('pad-3');
        }
        colHead.innerText = heading;
        count++;
    })

}
createTableSkeleton(tableElem);
// Append the completed table to the tableContainer in the DOM
...
let dataForBody = [{"id":1,"first_name":"Chadwick","last_name":"Ayre","email":"cayre0@cam.ac.uk","gender":"Male","img_src":"https://robohash.org/corporisquiaperiam.png?size=50x50&set=set1","class":11,"marks":18,"passing":false,"city":"Moorreesburg"},
...
const size = dataForBody.length;
const tbody = tableElem.createTBody();
const tableElemForMales = document.createElement('table');
fillTable(dataForBody, null);//for starting, load default table
function fillTable(bodyData, anotherBodyData){//load filtered data each time
    if(bodyData != null){
        tbody.innerHTML = '';
        if(anotherBodyData == null){
            tableElemForMales.innerHTML = '';
            tableElemForMales.classList.remove('marginTopAndBottom');
        } 
        for(let i=0; i<bodyData.length; i++){
            const row = tbody.insertRow();
            count = 0;
            const obj = bodyData[i];
            const tempObj = {};
            tempObj.id = obj.id;
            tempObj.Name = `${obj.img_src} * ${obj.first_name} ${obj.last_name}`;
            tempObj.Gender = obj.gender;
            tempObj.Class = obj.class;
            tempObj.Marks = obj.marks;
            if(obj.passing == true) tempObj.Passing = "Passing";
            else tempObj.Passing = "Failed";
            ...
            for(let key in tempObj){
                ...
                const col = document.createElement('td');
                    if(count == 0){
                        col.classList.add('pad-1');
                        col.append(tempObj[key]);
                    }else if(count == 1){
                        //splitIntoArray, createImg and append both name & img in col
                        col.classList.add('pad-2');
                        const tempArr = tempObj[key].split('*');
                        const tempContainer = document.createElement('span');
                        const tempImage = document.createElement('img');
                        tempImage.src = tempArr[0];
                        tempContainer.append(tempImage);
                        tempContainer.append(tempArr[1]);
                        col.append(tempContainer);
                    }
                    else{
                        col.classList.add('pad-3');
                        col.append(tempObj[key]);
                    }
                    count++;
                    row.append(col);
                    // col.innerHTML = tempObj[key];     
            }
        }
        tableContainer.append(tableElem);
    }//
   if(anotherBodyData != null){//handles male table in case of sortByGender
        
        createTableSkeleton(tableElemForMales);
        const tbody = tableElemForMales.createTBody();
        // tbody.innerHTML = '';
        for(let i=0; i<anotherBodyData.length; i++){
            const row = tbody.insertRow();
            count = 0;
            const obj = anotherBodyData[i];
            const tempObj = {};
            tempObj.id = obj.id;
            tempObj.Name = `${obj.img_src} * ${obj.first_name} ${obj.last_name}`;
            tempObj.Gender = obj.gender;
        ...
        tableContainer.append(tableElemForMales);
   }
}
...
//    event.dataTransfer.setData("text", val);
    localStorage.setItem('valueOfSearch', val);
    const word = val.trim().toLowerCase();
    if(word === '') fillTable(dataForBody, null)
    else filterOut();   
}

function handleSearchClick(event){
   filterOut();//whether you click or you change input(dono mei filter hoga)
}
...
for (let i = 0; i < dataForBody.length; i++) {
    const fullName = `${dataForBody[i].first_name} ${dataForBody[i].last_name}`.trim().toLowerCase();
    const email = dataForBody[i].email.trim().toLowerCase();
    if (fullName.includes(word) || email.includes(word)) {
        newData.push(dataForBody[i]); // Add object to the Set
    }
}
fillTable(newData, null);
}
...
for(let i=1; i<buttons.length; i++){
    buttons[i].addEventListener('click', (event)=>{
        ...
        const inside = clickedButton.innerHTML.toLowerCase();
        // -1 => do nothing, +1 => swap
        if(inside.includes("a-")){
           //ascending order of full name
           const newArr = [...dataForBody];
           newArr.sort((a,b) =>{
            ...
            fillTable(newArr, null);
        }else if(inside.includes("z-")){
            //desc order of full name
            const newArr = [...dataForBody];
           newArr.sort((a,b) =>{
                if(a.first_name > b.first_name){
                    return -1;
            ...
            fillTable(newArr, null);
        }else if(inside.includes("marks")){
            // means ascending order of marks
            const newArr = [...dataForBody];
           newArr.sort((a,b) =>{
        ...
        }else if(inside.includes("passing")){
            // passing means only show students who are passing

            const newArr = dataForBody.filter((data)=>{
                return data.passing;//cuz passing is boolean so if passing then return
            })
            fillTable(newArr, null);
            
        }else if(inside.includes("class")){
            // class means in the ascending order of class
            const newArr = [...dataForBody];
            newArr.sort((a,b) =>{
        ...
        }else if(inside.includes("gender")){
            //sort by gender, you show 2 tables, one containing all 
            //female students and the other containing all male students
            // Both should be shown, show them one below each other.
            const newArr = [...dataForBody];
           newArr.sort((a,b) =>{
            ...
            const arr2 = [];
            newArr.forEach((elem) =>{
                const gender = elem.gender.toLowerCase();
                // Check if gender contains "male" and does not contain "female"
                if (gender.includes("male") && !gender.includes("female")) {
                    arr2.push(elem); // add in male array
                }
                else{//female or other genders
                    arr1.push(elem);
                }
            })
            arr1.sort((a,b) =>{
